...
I initially received an error when I tried to load the tidyverse because the package was not installed. I installed it using `install.packages("tidyverse")`, ran `library(tidyverse)` again, and it loaded successfully. I did not encounter any additional problems importing the students.csv data.
## AI Use Disclosure

I used ChatGPT as a step-by-step tutor while learning the RStudio workflow so I could follow along and take personal notes. I entered and ran each command in RStudio myself. ChatGPT clarified where code belongs in an R Markdown block, showed me the basic commands requested by the assignment, checked my results, and helped me troubleshoot the missing tidyverse package. I did not use AI to conduct original statistical analysis or interpret the dataset.

My prompts included asking where to enter the code beneath `{r}`, whether the green arrow was the Run button, what to do after R reported that tidyverse was not installed, and whether the sample favorite-food and Student ID fields required my personal information. I also provided screenshots of my progress for confirmation after each step.
